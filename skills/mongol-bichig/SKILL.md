...
The companion **gege-linter** skill (same repo) covers the
`@gege-mn/gege-linter` library + CLI that mechanize all of the hard rules
above — reach for it instead of hand-rolling checks.

## References (progressive disclosure)

Read on demand; `references/` mirrors the repo's verified `docs/`:
...
| `references/README.md` | index mapping each doc to linter rules |
